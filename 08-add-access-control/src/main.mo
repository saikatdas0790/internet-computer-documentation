...
shared({ caller = initializer }) actor class() {
  ...
  stable var roles: AssocList.AssocList<Principal, Role> = List.nil();
  stable var role_requests: AssocList.AssocList<Principal, Role> = List.nil();
  ...
  func get_role(pal: Principal): ?Role {
    if (pal == initializer) {
      ...
      return AssocList.find<Principal, Role>(roles, pal, principal_eq);
    };
  };
  ...
  public shared({ caller }) func my_role_request(): async ?Role {
    return AssocList.find<Principal, Role>(role_requests, caller, principal_eq);
  };
